...
/*   Updated: 2021/04/20 17:04:36 by mteerlin      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	rt_parse_square(const char **line, t_scene *scene)
{
	...
	normalize = 1 / rt_vect_mag(scene->origin, newsq->o_vect);
	newsq->o_vect = rt_vect_scale(normalize, *newsq->o_vect);
...
bool	rt_sq_intersect(t_camera *cam, t_square *sq, t_scene *sc, double *t)
{
	...
	if (tt < *t)
	{
		sc->origin = rt_parse_vector("0.0,0.0,0.0");
		y = rt_parse_vector("1.0,0.0,0.0");
		if (fabs(rt_vect_dot(y, sq->o_vect)/rt_vect_mag(sc->origin, sq->o_vect)) > 0.75)
			y = rt_parse_vector("0.0,1.0,0.0");
